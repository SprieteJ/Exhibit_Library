...
volume. Plots the ratio over time as a single line.
...
def plot(ratio: pd.Series, n_binance: int, n_bybit: int):
    ...
    ax.fill_between(vis.index, vis.values, alpha=0.10, color=C_GREEN, zorder=2)
    ax.plot(vis.index, vis.values,
            color=C_GREEN, linewidth=1.8, zorder=3,
            label="Perp/spot volume ratio")

    # annotate latest
    last_val  = vis.iloc[-1]
    last_date = vis.index[-1]
    ax.scatter([last_date], [last_val], color=C_GREEN, s=40, zorder=5, linewidths=0)
    ann = ax.annotate(
        f"  {last_val:.2f}×",
    ...
    _apply_font(ann, FONT_SAANS, 8, "bold")
    ann.set_color(C_GREEN)
