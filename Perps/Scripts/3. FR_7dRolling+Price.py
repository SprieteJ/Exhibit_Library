"""
BTC Perpetual Funding Rate — 7D EMA Annualised + Hourly Spot Price
───────────────────────────────────────────────────────────────────
Left Y  : 1D annualised funding rate (dotted, light)
          + 7D EMA annualised funding rate (bold)
Right Y : Hourly spot price from Binance klines

EMA (span=7) replaces simple rolling mean to reduce lag.
Price plotted as raw USD level on 1h granularity.
...
def fetch_binance_price(symbol: str) -> pd.DataFrame:
    ...
    return df[["timestamp", "close"]].dropna().reset_index(drop=True)
...
def plot(funding: pd.DataFrame, price: pd.DataFrame, exchanges: list[str]):
    ...
    # ── RIGHT: hourly spot price
    ax_r.plot(dates_p, price_lvl,
              color=C_BLUE, linewidth=1.2, zorder=3, alpha=0.80,
              label=f"{SYMBOL_PRICE} price")
...
if __name__ == "__main__":
    ...
    print(f"  Latest {SYMBOL_PRICE} price    : ${last_p['close']:,.0f}")
