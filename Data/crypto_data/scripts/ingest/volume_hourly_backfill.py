#!/usr/bin/env python3
"""
volume_hourly_backfill.py
-------------------------
Pulls 90 days of hourly trading volume from CoinGecko Pro
for all assets in the registry.

Run once to initialise. Safe to re-run — deduplicates on (timestamp, coingecko_id).

Output → volume/hourly/volume_hourly.csv
...
# ── Fetch ─────────────────────────────────────────────────────────────────────
def fetch_volume_hourly(
    session: requests.Session,
    cg_id: str,
    api_key: str,
) -> pd.DataFrame:
    """90 days hourly via /coins/{id}/market_chart?days=90&interval=hourly"""
...
def main():
    ...
    print(f"\n[volume_hourly_backfill] {len(ids)} assets | 90 days hourly\n")
    ...
    for i, cg_id in enumerate(ids, 1):
        ...
        try:
            df = fetch_volume_hourly(session, cg_id, api_key)
            if df.empty:
                print("no data")
                failed.append(cg_id)
            ...
            write("volume_hourly", df, source=SOURCE)
            print(f"{len(df)} rows")
    ...
    print(f"\n[done] volume hourly backfill complete")
    if failed:
