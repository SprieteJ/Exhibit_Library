...
price_backfill.py
-----------------
Pulls FULL history of daily USD prices from CoinGecko Pro
for all assets in the registry.

Run once to initialise. Safe to re-run — deduplicates on (timestamp, coingecko_id).

Output → /home/jasperdemaere/Master_JDM/Exhibit_Library/Data/crypto_data/price/daily/price_daily.csv
...
def main():
    ...
    print(f"\n[price_backfill] {len(ids)} assets to fetch\n")
    ...
    for i, cg_id in enumerate(ids, 1):
        ...
        try:
            df = fetch_price_max(session, cg_id, api_key)
            if df.empty:
                print("no data")
                failed.append(cg_id)
            ...
            write("price_daily", df, source=SOURCE)
            print(f"{len(df)} rows")
